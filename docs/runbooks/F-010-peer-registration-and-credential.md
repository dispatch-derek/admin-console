...
credential rides the existing single POST per peer, as the HTTP header `X-Event-Bus-Peer-Auth-Token`, carrying
...
3. Confirm on the cwa side that the ingest endpoint **accepted** (2xx) the delivery — the request
   ...
   the `X-Event-Bus-Peer-Auth-Token` credential header.
