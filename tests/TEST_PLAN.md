...
`X-Event-Bus-Peer-Auth-Token` header is ever attached, no `EVENT_BUS_PEER_AUTH_TOKEN` config key exists, no
...
- **Note on which failures are "genuinely F-010 RED" vs. "passes now by legitimately reusing
  ...
  `X-Event-Bus-Peer-Auth-Token` header already sees "no header at all" as a mismatch regardless of F-010's
