...
# Start the relay as a supervised process:
DB_PATH=/data/console.db EVENT_BUS_URL=http://peer:8080/api/events node dist/relay/index.js
```
...
Events are delivered to peers over plain HTTP (if configured) with no authentication, no HMAC/signature, and no TLS enforcement. An MITM or an untrusted peer can read or forge `admin.*` events. **Mitigation:** enforce HTTPS-only in your peer list (reject non-`https` URLs in config), run peers on private networks, and use mTLS or bearer tokens if available. This is a trusted-network assumption (the same assumption as the BFF's engine adapter, which also has no TLS requirement at this stage).
...
- [ ] Peers are on a private network or use HTTPS/mTLS (defend against MITM of `admin.*` events).
