...
describe('HttpPeerTransport — a partial-ack re-drive still attaches the credential to the re-POST of the still-pending peer only', () => {
  it('peer A acks (200) on the first attempt, peer B permanently rejects (403); re-driving the SAME deliveryId without release() re-POSTs ONLY peer B, and that re-POST still carries X-Event-Bus-Peer-Auth-Token', async () => {
    ...
    expect(lowerHeaders(rejecter.requests[1]!)['x-event-bus-peer-auth-token']).toBe('partial-ack-secret');
  });
});
