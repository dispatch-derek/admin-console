...
describe.skipIf(!HttpPeerTransport)('REQ-F010-004/005 — the credential is carried as the third wire element, on the SAME POST, verbatim', () => {
  it('with a credential configured, the outbound POST carries all three application-level headers: content-type, x-event-delivery-id, X-Event-Bus-Peer-Auth-Token', async () => {
    ...
    expect(h['x-event-bus-peer-auth-token']).toBe('super-secret-token');
  });

  it('F-010 introduces EXACTLY ONE new application-level header vs. the pre-F-010 (no-credential) baseline — the header-NAME-SET differs by exactly {x-event-bus-peer-auth-token}', async () => {
    ...
    expect(added).toEqual(['x-event-bus-peer-auth-token']);
  });
  ...
  async function captureTransportHeaderValue(peerAuthToken: string): Promise<string | null | undefined> {
    ...
    try {
      ...
      if (headersArg instanceof Headers) return headersArg.get('X-Event-Bus-Peer-Auth-Token');
      if (headersArg && typeof headersArg === 'object') {
        const rec = headersArg as Record<string, string>;
        return rec['X-Event-Bus-Peer-Auth-Token'] ?? rec['x-event-bus-peer-auth-token'];
      }
      return undefined;
    } finally {
  ...
  it('with NO credential configured (constructor arg omitted entirely), the outbound POST carries only the two pre-F-010 headers — no X-Event-Bus-Peer-Auth-Token at all (REQ-F010-005 scoping: the three-header MUST applies only "when a credential is configured")', async () => {
    const p = await startFakePeer(200);
    peers = [p];
  ...
  it('a WHITESPACE-ONLY credential (" ") IS attached as a header (the KEY is present at the peer, proving the transport did not treat it as absent) even though undici/fetch\'s own Headers normalization empties its VALUE in transit — DOCUMENTED, not a spec violation (REQ-F010-005\'s own text: a peer observing trimmed whitespace "does not by itself prove a spec violation")', async () => {
    ...
    const h = lowerHeaders(p.requests[0]!);
    expect(Object.keys(h)).toContain('x-event-bus-peer-auth-token'); // attached, not omitted (contrast with the "" -> absent case above)
    ...
    expect(h['x-event-bus-peer-auth-token']).toBe('');
  });
});
...
describe.skipIf(!HttpPeerTransport)('REQ-F010-009 — a single configured credential is applied to EVERY configured peer (default: one shared secret)', () => {
  it('two distinct peers both receive the IDENTICAL credential header value', async () => {
    ...
    expect(lowerHeaders(p1.requests[0]!)['x-event-bus-peer-auth-token']).toBe('shared-secret');
    expect(lowerHeaders(p2.requests[0]!)['x-event-bus-peer-auth-token']).toBe('shared-secret');
  });
});
